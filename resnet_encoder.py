...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        self.inplanes = int(64 * width_multiplier)
        self.dilation = 1
...
def _resnet_encoder(
    ...
    **kwargs: Any,
    ...
    model = ResNetEncoder(block, layers, **kwargs)

    return model


def resnet18Encoder(block=BasicBlock, **kwargs: Any) -> ResNetEncoder:
    ...
    return _resnet_encoder(block=block, layers=[2, 2, 2, 2], **kwargs)


# test
# x = torch.randn((1, 3, 32, 32))

# model = resnet18Encoder(block=BasicBlock)
# model.eval()
# with torch.no_grad():
#     out = model(x)
# print(out.shape)